...
public class SimpleScreens {
     ...
     * @param hasCrashed 
     */
	void redrawFinish(MazePanel panel, int pathlen, float energyConsumed, float initialEnergy, boolean hasCrashed) {
		Graphics g = panel.getBufferGraphics() ;
        if (null == g) {
        	LOGGER.warning(errorMsg) ;
        }
        else {
            redrawFinish(g, pathlen, energyConsumed, initialEnergy, hasCrashed);
        }
	}
	/**
	 ...
	 */
	private void redrawFinish(Graphics gc, int pathlen, float energyconsum, float initialEnergy, boolean hasCrashed) {
		if (hasCrashed) {
			...
			updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
			centerString(gc, "The robot crashed or ran out of energy.", 160);
			// write the path length
			updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
			centerString(gc, "The robot traveled a path of length: " + pathlen, 200);
			// write the energy consumption
			updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
			centerString(gc, "Energy used: " + energyconsum + "/" + initialEnergy , 240);
			// write the instructions
			gc.setColor(ColorTheme.getColor(MazeColors.TITLE_DEFAULT));
			centerString(gc, "Hit any key to try again", 300);
		} else {
			// produce blue background
			drawBackground(gc);
			// write the title 
			...
			// write the energy consumption
			updateFontAndColor(gc, smallBannerFont, MazeColors.TITLE_SMALL);
			centerString(gc, "Energy consumed: " + energyconsum, 240);
			// write the instructions
			gc.setColor(ColorTheme.getColor(MazeColors.TITLE_DEFAULT));
