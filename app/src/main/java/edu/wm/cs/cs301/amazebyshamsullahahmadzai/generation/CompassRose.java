...
public class CompassRose {
    ...
    private static final float MAIN_LENGTH = 1.0f;
    private static final float MAIN_WIDTH = 0.3f;
    ...
    private final double markerRadius;
...
//        g.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
         ...
         */
        drawBackground(g);
        drawArms(g, armLength, armWidth);
        drawBorderCircle(g, width); // note: not currently visible due to color settings
	...
	private void drawArms(final MazePanel g2, final int length, final int width) {
        ...
        g2.setColor(ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MAIN_COLOR).toArgb());
        // draw each arm
	...
	private void drawBackground(final MazePanel g2) {
		...
		g2.setColor(ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_BACKGROUND).toArgb());
	...
	private void drawBorderCircle(MazePanel g2, int width) {
		...
		// draw both arcs
		g2.setColor(ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_CIRCLE_SHADE).toArgb());
        g2.addArc(x, y, w, w, 45, 180);
        g2.setColor(ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_CIRCLE_HIGHLIGHT).toArgb());
	...
	private void drawDirectionMarker(MazePanel g2, int width) {
		...
		g2.setColor((CardinalDirection.South == currentDir) ? 
				ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MARKER_COLOR_CURRENTDIRECTION).toArgb() :
					ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MARKER_COLOR_DEFAULT).toArgb());
		drawMarker(g2, centerX - 10, centerY - offset + 10, "N");

		g2.setColor((CardinalDirection.East == currentDir) ? 
				ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MARKER_COLOR_CURRENTDIRECTION).toArgb() :
					ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MARKER_COLOR_DEFAULT).toArgb());
		drawMarker(g2, centerX + offset - 5, centerY + 8, "E");
		...
		g2.setColor((CardinalDirection.North == currentDir) ? 
				ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MARKER_COLOR_CURRENTDIRECTION).toArgb() :
					ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MARKER_COLOR_DEFAULT).toArgb());
		drawMarker(g2, centerX - 8, centerY + offset + 5, "S");

		g2.setColor((CardinalDirection.West == currentDir) ? 
				ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MARKER_COLOR_CURRENTDIRECTION).toArgb() :
					ColorTheme.getColor(ColorTheme.MazeColors.COMPASSROSE_MARKER_COLOR_DEFAULT).toArgb());
		drawMarker(g2, centerX - offset - 5, centerY + 8, "W");
