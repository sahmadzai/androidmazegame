...
public class ColorTheme {
	...
	//specifically for CompassRose.java, corresponding enum values have prefix COMPASS ROSE_
	...
	// requires predefined color for white (seen), grey for other walls, red for current location, yellow for solution
	...
	private static class ColorSettingsAdvanced extends ColorSettings {
		 ...
		 * @param color top is true for the top rectangle, false for the bottom
		...
		Color getColor(MazeColors color, float percentToExit) {
			Color result = (MazeColors.BACKGROUND_TOP == color)?
					...
					blend(Color.valueOf(Color.DKGRAY), greenWM, percentToExit);
			LOGGER.log(Level.FINEST, "given:" + color + ", returns color: " + result);
			return result;
		}
		/**
		...
		private Color blend(Color fstColor, Color sndColor, double weightFstColor) {
			...
			if (weightFstColor > 0.95)
				return fstColor;
			double r = weightFstColor * red(fstColor.toArgb()) + (1-weightFstColor) * red(sndColor.toArgb());
			double g = weightFstColor * green(fstColor.toArgb()) + (1-weightFstColor) * green(sndColor.toArgb());
			double b = weightFstColor * blue(fstColor.toArgb()) + (1-weightFstColor) * blue(sndColor.toArgb());
			double a = Math.max(alpha(fstColor.toArgb()), alpha(sndColor.toArgb()));
